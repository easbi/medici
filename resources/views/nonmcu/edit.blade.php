            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="card border-0 shadow rounded">
                        <div class="card-body">
                            <form action="{{ route('nonmcu.update', $pemeriksaannonmcu->id) }}" method="POST">                        
                                @csrf
                                @method('PUT')
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control" name="id_user_diperiksa">
                                        @foreach($nama_pegawai as $pegawai)
                                            <option value={{$pegawai->id }} {{$pegawai->id == $pemeriksaannonmcu->id_user_diperiksa ? 'selected="selected"' : ''}}>{{$pegawai->nama}}</option>
                                        @endforeach                                        
                                    </select> 
                                </div>    
                                <div class="form-group">
